...
fn run(
  ...
  'gameloop: loop {
    ...
    println!("time_count: {:.2}", time_count.elapsed().as_secs_f64());
    ...
    match loaded_map {
      Map::Overworld => {
        ...
        for i in &spawnable_areas{
          ...
          //println!("{:?}",test_result);
          let elapsed = time_count.elapsed();
          if test_result == true && random_spawn() 
            && (((elapsed.as_secs_f64() * 100.0).round()) % ((DELTA_TIME* 100.0).round()) == 0.0 ){
            println!("check time step: {:.2}", elapsed.as_secs_f64());
